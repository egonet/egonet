...
public class StatisticsFrame extends JPanel
{
   ...
   private void jbInit() throws Exception
   {
     ...
     Iterator questions = EgoClient.study.getQuestionOrder(Question.ALTER_PAIR_QUESTION).iterator();
      ...
      if (studyStatable)
      {
         alterQuestionMenu.setSelectedIndex(0);
         stats = EgoClient.interview.generateStatistics((Question) alterQuestionMenu.getSelectedItem());

         // Use stats to initialize panels
         summaryPanel   = new StatisticsArrayPanel(new InterviewSummaryModel(stats));
         ...
         componentPanel = new StatisticsArrayPanel(new CompositionalStatsModel(stats));
          ...
          * Layout
          *********************************************************************/
         panel.add(tabs, new GridBagConstraints(0, 0, 1, 1, 1.0, 0.9, GridBagConstraints.CENTER,
               GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
         panel.add(alterQuestionMenu, new GridBagConstraints(0, 1, 1, 1, 0.2, 0.1, GridBagConstraints.SOUTHEAST,
               GridBagConstraints.NONE, new Insets(10, 10, 10, 10), 6, 6));

         /**********************************************************************
          * Event Handlers
          *********************************************************************/
         alterQuestionMenu.addActionListener(new java.awt.event.ActionListener() {
         ...
         EgoClient.frame.saveAlterSummary.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
         ...
         EgoClient.frame.saveTextSummary.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
         ...
         EgoClient.frame.saveWeightedAdjacencyMatrix.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
         ...
         EgoClient.frame.saveAdjacencyMatrix.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
         ...
         EgoClient.frame.saveInterviewStatistics.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
      ...
      else
      {
         ...
         this.add(new JLabel("No questions with adjacent and non-adjacent selections found."));
      }

      EgoClient.frame.close.addActionListener(new java.awt.event.ActionListener() {
         public void actionPerformed(ActionEvent e)
         {
   ...
   void updateAll()
   {
      for (int i = 0; i < tabs.getTabCount(); i++)
      {
         ...
         {
            ((StatisticsArrayPanel) component).getTableModel().setStats(stats);
   ...
   void saveTextSummary_actionPerformed(ActionEvent e)
   {
      ...
      PrintWriter w = EgoClient.storage.newStatisticsPrintWriter("Text Summary", "txt", filename);
   ...
   void saveAdjacencyMatrix_actionPerformed(ActionEvent e)
   {
      ...
      PrintWriter w = EgoClient.storage.newStatisticsPrintWriter("Adjaceny Matrix", "csv", filename);
   ...
   {
      String[] name = EgoClient.interview.getName();
      String filename = name[0] + "_" + name[1] + "_Weighted_Adjacency_Matrix";
      PrintWriter w = EgoClient.storage.newStatisticsPrintWriter("Adjaceny Matrix", "csv", filename);
   ...
   void saveInterviewStatistics_actionPerformed(ActionEvent e)
   {
      ...
      try
      {
         if (q != null)
         {
            EgoClient.storage.writeStatisticsFiles(stats, EgoClient.interview.getName());
         }
      }
 ...
 * Initial Checkin
 *
 * Revision 1.9  2004/04/11 00:24:48  admin
 * Fixing headers
 *
 * Revision 1.8  2004/04/06 20:29:22  admin
 * First pass as supporting interactive applet linking interviews
 *
 * Revision 1.7  2004/04/01 15:11:16  admin
