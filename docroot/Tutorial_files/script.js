...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "Tutorial_files";
var g_showoutline = 0;
...
if ( IsFramesMode() ) {
  ...
  else if( ! ( ( OUTLINE_FRAME == frame && true)  || (NOTES_FRAME == frame && !g_shownotes ) ) ){
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			window.location.replace(  "Tutorial_files/error.htm" );
	}
	else {
