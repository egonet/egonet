   ...
   extends ELSAction
{
                                ...
                                HttpServletResponse response) throws IOException, ServletException
   {
      ...
      logger.debug("EgoNameAction::perform for " + form.getFirstName() + " " + form.getLastName());
      ...
      {
         logger.error("Error looking up interview", e);
         throw new IOException(e.getMessage());
      }

      ActionForward nextActionForward = null;
      if (position.getGlobalPosition() == 0)
      {
         logger.debug("Moving you into the tutorial");
         nextActionForward = mapping.findForward(WebShared.FORWARD_TUTORIAL);
      }
      else
      {
         logger.debug("Moving you forward to the next position");
         nextActionForward = mapping.findForward(WebShared.FORWARD_POSITION);
      }
      
      if(nextActionForward != null)
    	  logger.debug("Next forward mapping: " + nextActionForward);
      else
    	  logger.debug("nextActionForward was null. Struts couldn't find the action!");
   ...
   public InterviewSBLocal getInterviewSB()
   {
      ...
      if (_interviewSB == null)
      {
         ...
         {
            logger.error("Exception during getInterviewSB(): " + e);
         }
      }
